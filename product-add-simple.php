...
if (empty($opts) || empty($opts['sku']) || empty($opts['name']) || empty($opts['price'])) {
	echo "Usage: product-add-simple.php --sku SKU --name NAME --price PRICE [--qty QTY] [--cat CATEGORY_ID] [--summary SUMMARY] [--desc  DESCRIPTION] [--weight WEIGHT] [--store STORE_ID] [--set ATTRIBUTE_SET_ID]\n";
	exit(1);
}
